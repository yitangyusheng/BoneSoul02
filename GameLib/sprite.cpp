...
namespace GameLib
{
    ...
    float Sprite::textout(ID3D11DeviceContext* context,
        ...
        bool world) const
    {
        return textout(context, s, VECTOR2(x, y), VECTOR2(sx, sy), VECTOR4(r, g, b, a), world);
    }
        ...
        const VECTOR4 &color) const
    {
        ...
        vertex vertices[4];
#ifdef GAMELIB_PLUS_UP
        vertices[0] = { VECTOR3(v[3].x, v[3].y, 0), color, VECTOR2(0, 0) };
        vertices[1] = { VECTOR3(v[2].x, v[2].y, 0), color, VECTOR2(1, 0) };
        vertices[2] = { VECTOR3(v[0].x, v[0].y, 0), color, VECTOR2(0, 1) };
        vertices[3] = { VECTOR3(v[1].x, v[1].y, 0), color, VECTOR2(1, 1) };
#else
        vertices[0] = { VECTOR3(v[3].x, v[3].y, 0), color, VECTOR2(0, 1) };
        vertices[1] = { VECTOR3(v[2].x, v[2].y, 0), color, VECTOR2(1, 1) };
        vertices[2] = { VECTOR3(v[0].x, v[0].y, 0), color, VECTOR2(0, 0) };
        vertices[3] = { VECTOR3(v[1].x, v[1].y, 0), color, VECTOR2(1, 0) };
#endif
    ...
    void Sprite::draw_triangles_indexed(ID3D11DeviceContext* context,
        const VECTOR3* pVertex, const VECTOR4* pColor, const VECTOR2* pUvs,
        short* pIndices, int nVert, int nPolygon) const 
    {
        D3D11_VIEWPORT viewport;
        UINT numViewports = 1;
        context->RSGetViewports(&numViewports, &viewport);
    ...
    void Sprite::draw_triangle_fan_indexed(ID3D11DeviceContext* context,
        const VECTOR3* pVertex, const VECTOR4* pColor, const VECTOR2* pUvs,
        short* pIndices, int nPolygon) const
    {
        D3D11_VIEWPORT viewport;
        UINT numViewports = 1;
        context->RSGetViewports(&numViewports, &viewport);
